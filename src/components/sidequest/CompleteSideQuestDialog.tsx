...
import { Sparkles, X, Clock, ImagePlus } from "lucide-react";
...
const EVIDENCE_ERROR = "Para enviar a Sidequest, envie uma foto ou escreva um comentário com pelo menos 50 caracteres contando o que você fez.";
...
export const CompleteSideQuestDialog = ({ quest, open, onOpenChange, busy, onConfirm }: Props) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!busy) { onOpenChange(v); if (!v) reset(); } }}>
      <DialogContent
        className={`max-w-md max-h-[calc(100dvh-1rem)] p-0 overflow-y-auto border-0 rounded-[2rem] bg-gradient-to-br ${cat.gradient} shadow-glow ring-2 ${cat.ring}`}
      >
        <div className="rounded-[1.85rem] bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-100/90 p-5 sm:p-6 relative overflow-visible min-h-0">
          ...
          <div className="text-center">
            <div className={`inline-flex items-center gap-1 rounded-full px-3 py-0.5 text-[11px] font-bold ${cat.badge} shadow-sm`}>
              <Sparkles className="w-3 h-3" /> Enviar para aprovação
            </div>
            ...
            <div>
              ...
              {preview ? (
                <div className="mt-1 relative rounded-2xl overflow-hidden ring-2 ring-amber-200 bg-white/60">
                  <img src={preview} alt="Prévia" className="w-full max-h-[280px] object-contain" />
                  <button
                    type="button"
          ...
          <div className="sticky bottom-0 -mx-5 sm:-mx-6 px-5 sm:px-6 py-3 bg-amber-50/95 backdrop-blur border-t border-amber-200/70 flex flex-col sm:flex-row gap-2 justify-center">
            <Button
              variant="ghost"
          ...
          <p className="text-center text-[10px] text-amber-900/60 mt-2">
            Um responsável vai revisar sua Side Quest antes dos Auris aparecerem ✨
          </p>
        </div>
      </DialogContent>
    </Dialog>
