  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {list.map(s => (
          <Card key={s.id} className="border-0 shadow-card rounded-2xl overflow-hidden">
            <CardContent className="p-0">
              {s.photo_url && (
                <img src={s.photo_url} alt={s.activity?.name || "Prova"} className="w-full h-56 object-cover bg-muted" loading="lazy" />
              )}
              <div className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge className="bg-gradient-reward text-accent-foreground border-0">{formatBRL(s.reward_amount_cents)}</Badge>
                </div>
                ...
                {tab === "pendente" && (
                  <>
                    ...
                    <div className="flex gap-2">
                      <Button variant="success" className="flex-1" onClick={() => review(s, "aprovado")}>
                        <Check className="w-4 h-4" /> Aprovar
                      </Button>
                      <Button variant="destructive" className="flex-1" onClick={() => review(s, "recusado")}>
        ...
        {list.length === 0 && (
          <p className="text-muted-foreground text-sm col-span-full">Nenhuma submissão {tab === "pendente" ? "pendente" : tab === "aprovado" ? "aprovada" : "recusada"}.</p>
        )}
      </div>
    </div>
  );
