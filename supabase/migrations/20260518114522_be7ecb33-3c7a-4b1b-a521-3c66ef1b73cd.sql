-- Estende o enum cosmetic_category com chest e especial
ALTER TYPE public.cosmetic_category ADD VALUE IF NOT EXISTS 'chest';
ALTER TYPE public.cosmetic_category ADD VALUE IF NOT EXISTS 'especial';

-- Coluna para indicar qual item o baú revela ao abrir
ALTER TABLE public.cosmetic_items
  ADD COLUMN IF NOT EXISTS chest_reveals_item_id uuid REFERENCES public.cosmetic_items(id) ON DELETE SET NULL;

-- Atualiza evaluate_cosmetic_unlocks: ao desbloquear um chest com chest_reveals_item_id, desbloqueia também o item-alvo
...
BEGIN
  ...
  -- CHEST REVEAL: para cada chest desbloqueado que aponta para um item-alvo, desbloqueia também o item-alvo
  INSERT INTO public.child_unlocked_items (child_id, item_id, source)
  SELECT _child_id, chest.chest_reveals_item_id, 'chest'
  FROM public.child_unlocked_items cu
  JOIN public.cosmetic_items chest ON chest.id = cu.item_id
  JOIN public.cosmetic_items target ON target.id = chest.chest_reveals_item_id
  WHERE cu.child_id = _child_id
    AND chest.category = 'chest'
    AND chest.chest_reveals_item_id IS NOT NULL
